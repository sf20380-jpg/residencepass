...
export default function GuardDashboard() {
  ...
  function getStatusBadge(status: string) {
    const styles: any = {
      pending: { background: '#E6F1FB', color: '#185FA5', label: 'Pending' },
      inside: { background: '#EAF3DE', color: '#3B6D11', label: 'Inside' },
      checked_out: { background: '#F1EFE8', color: '#5F5E5A', label: 'Out' },
    }
    const s = styles[status] || styles.pending
    return (
      <span style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '20px', fontWeight: '500', background: s.background, color: s.color }}>
        {s.label}
      </span>
    )
  }
  ...
  return (
    <div style={{ maxWidth: '420px', margin: '0 auto', minHeight: '100vh', fontFamily: 'sans-serif', background: '#F8F9FA' }}>

      {/* Topbar */}
      <div style={{ background: '#fff', borderBottom: '1px solid #eee', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        <h2 style={{ flex: 1, fontSize: '16px', fontWeight: '500', margin: '0' }}>Dashboard</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', padding: '16px 16px 0' }}>
        {[
          ...
          <div key={i} style={{ background: '#fff', border: '1px solid #eee', borderRadius: '8px', padding: '12px 10px', textAlign: 'center' }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', background: '#fff', borderBottom: '1px solid #eee', marginTop: '16px' }}>
        {[['today', 'Today'], ['inside', 'Inside'], ['history', 'History']].map(([key, label]) => (
          <button key={key} onClick={() => setActiveTab(key)}
            style={{ flex: 1, padding: '12px 0', fontSize: '13px', background: 'none', border: 'none', borderBottom: activeTab === key ? '2px solid #185FA5' : '2px solid transparent', color: activeTab === key ? '#185FA5' : '#888', fontWeight: activeTab === key ? '500' : '400', cursor: 'pointer' }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '12px 16px 0' }}>
        <div style={{ position: 'relative' }}>
          <span style={{ position: 'absolute', left: '11px', top: '50%', transform: 'translateY(-50%)', color: '#aaa', fontSize: '14px' }}>🔍</span>
          <input
            type="text"
            placeholder="Search by name, unit or plate..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ width: '100%', padding: '10px 12px 10px 32px', border: '1px solid #eee', borderRadius: '8px', fontSize: '14px', background: '#fff', boxSizing: 'border-box' }} />
        </div>
      </div>
      ...
      <div style={{ padding: '12px 16px 100px' }}>
        {loading ? (
          <p style={{ textAlign: 'center', color: '#888', fontSize: '13px', padding: '32px' }}>Loading...</p>
        ) : currentList.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#888', fontSize: '13px', padding: '32px' }}>No visitors found</p>
        ) : currentList.map((v: any) => (
          <div key={v.id} style={{ background: '#fff', border: '1px solid #eee', borderRadius: '12px', padding: '14px', marginBottom: '10px', cursor: 'pointer' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <h3 style={{ fontSize: '14px', fontWeight: '500', margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{v.name}</h3>
